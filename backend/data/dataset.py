...
# config 파일과 preprocessor 파일은 별도로 존재한다고 가정
...
class EnglishDialectsDataset(Dataset):
    ...
    def __init__(self, split='train', use_augment=False,
        ...
        try:
            ...
            # ✅ 오디오 컬럼을 Audio feature로 캐스팅
            if "audio" in self.dataset.features:
                self.dataset = self.dataset.cast_column(
                    "audio", 
                    Audio(sampling_rate=self.audio_sample_rate)
                )
            
            # 💡 [수정] 데이터 로드 후 데이터를 무작위로 섞어 레이블의 순차적 편향을 제거
    ...
    def _parse_label(self, config_name):
        try:
            ...
            # REGION_LABELS와 GENDER_LABELS가 0이 아닌 유효한 ID를 반환한다고 가정
    ...
    def __getitem__(self, idx):
        ...
        return {
            "waveform": waveform,
            "region_id": region_id,
            "gender_id": gender_id,  # auxiliary task
            "coords": coords,
